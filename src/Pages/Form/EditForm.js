...
import "../../../../rararurura/node_modules/antd/dist/antd.css";
import {
  loadDrugByID,
  loadLookupsByID,
} from "../../Redux/Formulary/EditFormFormularySlice";
        ...
        <Row gutter={[32, 32]}>
          <Col span={6}>
            ...
            <Select
              ...
              value={_drug?.ApprovedClassification?.map(
                (classification) =>
          ...
          <Col span={6}>
            ...
            <Select
              ...
              style={{ width: "100%" }}
              placeholder="Please select"
              value={_drug?.ApprovedDosingTypes?.map(
                (dosing) => dosing.DosingTypeID || dosing
              )}
              onChange={(value) => {
                handleChange(value, "Dosing");
              }}
            >
              {dosing?.map((dose, d) => (
                <Option value={dose.DosingTypeID} key={d}>
                  {dose?.DosingType}
                </Option>
              ))}
            </Select>
          ...
          <Col span={6}>
            ...
            <Select
              ...
              placeholder="Please select"
              onChange={(value) => {
                handleChange(value, "Routes");
              }}
              value={_drug?.ApprovedRoutes?.map(
                (routes) => routes.RouteID || routes
              )}
            >
              {routes?.map((route, r) => (
                <Option value={route.RouteID} key={r}>
                  {route?.Route}
                </Option>
              ))}
            </Select>
          ...
          <Col span={6}>
            ...
            <Select
              ...
              value={
                _drug?.FormularyDrug ? _drug?.FormularyDrug[0]?.FormId : ""
              }
            >
              {forms?.map((form, f) => (
                <Option value={form?.FormID} key={f}>
                  {form?.FormName}
                </Option>
              ))}
            </Select>
